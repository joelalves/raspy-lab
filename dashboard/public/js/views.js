// Renderers for the poll-driven status tiles: Overview, System, Docker,
// Jenkins, SonarQube, Weather, Pi-hole, Shelly. Pure HTML-string builders -
// main.js's refresh() loop calls these and injects the result into the DOM;
// nothing here touches the DOM directly itself.
import { formatBytes, formatDuration, formatAgo, formatUptime, pctStatus, tempStatus, formatCo2, formatBuildResult } from './format.js';
import { renderRadioOverviewCard } from './radio.js';
...
// grid of labeled values below - unlike plain card(), long summary text
// here has its own row instead of fighting the title for space.
...
export function renderDocker(section) {
...
export function renderJenkins(section) {
  if (section.error) return `<div class="error-msg">${section.error}</div>`;
...
export function renderSonarQube(section) {
  if (section.error) return `<div class="error-msg">${section.error}</div>`;
...
export function renderWeather(section) {
  if (section.error) return `<div class="error-msg">${section.error}</div>`;
...
export function renderPihole(section) {
  if (section.error) return `<div class="error-msg">${section.error}</div>`;
...
export function renderShelly(section) {
  if (section.error) return `<div class="error-msg">${section.error}</div>`;
